...
description: "MANDATORY when a task replays log lines against exact format rules (MALFORMED silently ignored vs REJECTED recorded). Hard time limit: your FIRST tool call must be ONE bash heredoc that writes the complete processing script AND runs it so both output files exist immediately; NEVER cat/print the logs (thousands of lines). Validate each RAW line with one anchored re.fullmatch after rstrip('\\n') ONLY — NEVER .strip(): stripping repairs a whitespace-defective line and wrongly applies it. AMOUNT regex is EXACTLY [1-9]\\d* — never 0*[1-9]\\d*, which accepts malformed leading-zero amounts like 0500. A line that MATCHES but names an unknown account, overdraws (spending exactly the balance is ALLOWED), or self-transfers is REJECTED and recorded in processing order — never silently dropped. After the files exist audit: grep -nE \"strip|0\\*\" script.py — only rstrip('\\n') allowed, no 0* in any regex; fix and re-run once if hit, then stop; do not re-read the logs."
---
